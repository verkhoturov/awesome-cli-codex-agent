import { resumeThread } from '@/adapters';
import { DEFAULT_REASONING_EFFORT } from '@/app/config.js';
import { agentProfile, type CliState, isReasoningEffort, isSandboxMode } from '@/core';
import type { ReasoningEffort } from '@/core/types.js';
import type { CommandContext, CommandResult } from './types.js';

export async function resumeSavedThread(
...
export function changeModel({ state, ui }: CommandContext, args: string[]): CommandResult {
  if (args.length === 0) {
    ui.emit({
      kind: 'status',
...
export function changePermissions({ state, ui }: CommandContext, args: string[]): CommandResult {
...
export async function logout({ ui }: CommandContext, args: string[]): Promise<CommandResult> {
  if (args.length > 0) {
...
export function resetConversation(state: CliState): void {
  state.conversation = {};
}
